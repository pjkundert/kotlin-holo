#+TITLE: Multi-Party Diffie-Hellman Key Exchange
#+author: Perry Kundert
#+email: perry@kundert.ca
#+date: 2025-01-07 08:04:28
#+draft: false
...
cryptographic secrets amongst multiple parties.  The Elliptic Curve Diffie-Hellman algorithm supports
...
In this paper we implement the examples in Wikipedia's [[https://en.wikipedia.org/wiki/Diffie-Hellman][Diffie-Hellman]] and [[https://en.wikipedia.org/wiki/Elliptic-curve_Diffie-Hellman][Elliptic-curve
Diffie-Hellman (ECDH)]] using the Python [[https://github.com/pjkundert/crypto-licensing.git][crypto-licensing]] module's Ed25519/X25519 implementation, and
propose enhancements to =lair-keystore= to support Holochain applications using these constructs;
including a robust method to ensure that multi-party Diffie-Hellman secret sharing does /not/ leak
the 2-party D-H secret between every pair of Agents -- which is precisely one of the "Intermediate"
value shared in the naive multi-party Diffie-Hellman computation! ([[./README-DH.pdf][PDF]], [[./README-DH.txt][Text]])
#+END_ABSTRACT
...
* Two-Party Shared Secrets
  ...
  In this simple [[https://en.wikipedia.org/wiki/Diffie-Hellman][Diffie-Hellman]] example using integers, a public key is the primitive root \( g \)
  raised to the power of the private key, modulo the prime \( p \):
  ...
  A shared secret calculation has a similar structure.  By substitution, we can see that our private
  key plus the counterparty's public key can derive a common power of \( g \) -- a "shared" secret
  value:
...
** Verify Eve's Known Values
   ...
   #+LATEX: {\scriptsize
   #+BEGIN_SRC python :session dhke :results value :exports results
     [
...
* Three-Party Shared Secret Implementation
  ...
  and sharing of values computed by having each party apply their private key exponent to the public
  ...
  However, in this example we'll demonstrate each party creating private keys \( a, b, c \), and
  transmitting the corresponding public keys \( A, B, C \) and all intermediate values to all
  counterparties.
...
** Computing N-Party Diffie-Hellman Shared Secret
   ...
   | Intermediates         |               Value |
   |-----------------------+---------------------|
   | Alice -> Bob          |     348467...458856 |
   | Alice -> Bob -> Carol |     578277...131494 |
   | Bob -> Carol          |     490540...933833 |
   | Bob -> Carol -> Alice |     203559...629626 |
   | Carol -> Alice        |     103956...065868 |
   | Carol -> Alice -> Bob |     497596...926908 |
   |-----------------------+---------------------|
   | Agent                 | Final Shared Secret |
   |-----------------------+---------------------|
   | Alice                 |     128121...994858 |
...
* Implementing in =lair-keystore=
  ...
  utilize ECDH (Elliptic Curve Diffie-Hellman) for Two-Party shared secrets, and support for
